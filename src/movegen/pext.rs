...
unsafe fn pext128(a: Bitboard, mask: Bitboard) -> u128 {
    let out0 = _pext_u64(a.lo_bits(), mask.lo_bits()) as u128;
    let out1 = _pext_u64(a.hi_bits(), mask.hi_bits()) as u128;
    out0 | (out1 << mask.lo_bits().count_ones())
}

pub fn get_lance_attacks_internal(sq: Square, occ: Bitboard, stm: u8) -> Bitboard {
    let idx = unsafe { pext128(occ, Bitboard(BLOCKER_MASKS[stm as usize][sq.as_usize()])) };
    LANCE_ATTACKS[sq.as_usize()][idx as usize][stm as usize]
}

pub fn get_rook_attacks_internal(sq: Square, occ: Bitboard) -> Bitboard {
    let idx = unsafe { pext128(occ, Bitboard(BLOCKER_MASKS[3][sq.as_usize()])) };
    ROOK_ATTACKS[sq.as_usize()][idx as usize]
}

pub fn get_bishop_attacks_internal(sq: Square, occ: Bitboard) -> Bitboard {
    let idx = unsafe { pext128(occ, Bitboard(BLOCKER_MASKS[2][sq.as_usize()])) };
